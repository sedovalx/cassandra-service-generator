...
import javax.lang.model.element.{ElementKind, ExecutableElement, Modifier}
import javax.lang.model.util.Types

import com.datastax.driver.mapping.annotations.{Accessor, Query, QueryParameters}
import com.squareup.javapoet._
import com.github.sedovalx.cassandra.domain.generation.cql.CqlBuilder
import com.github.sedovalx.cassandra.domain.generation.metadata.{KeyMetadata, TableMetadata}
import com.github.sedovalx.cassandra.domain.generation.utils.{JavaxModelHelper, MethodSpecUtils}
import com.github.sedovalx.cassandra.domain.generation.utils.SeqExtensions.{asParameterSpecs, extendedSeq}
...
class AccessorSpecBuilder(metadata: TableMetadata, types: BuildTypes, typeUtils: Types) {
    ...
    private def buildMethodSpec(element: ExecutableElement, anno: Query): MethodSpec.Builder = {
        val paramSpecs = element.getParameters.asScala.map { it =>
            ...
            val paramType = ClassName.get(JavaxModelHelper.asTypeElement(it.asType(), typeUtils))
        ...
        MethodSpec.methodBuilder(element.getSimpleName.toString)
            ...
            .returns(ClassName.get(JavaxModelHelper.asTypeElement(element.getReturnType, typeUtils)))
            .addAnnotation(
                AnnotationSpec.builder(classOf[Query])
                    .addMember("value", anno.value())
                    .build()
            )
    }
}
